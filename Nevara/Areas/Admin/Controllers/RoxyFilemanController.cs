...
namespace Nevara.Areas.Admin.Controllers
{
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        public RoxyFilemanController(IHostingEnvironment env)
        {
            ...
            _filesRootPath = "/wwwroot/images";
            _filesRootVirtual = "/images";
        ...
        #region API Actions
        public async Task<IActionResult> DIRLIST(string type)
        {
            try
            {
                ...
                ArrayList dirs = await Task.Run(() => ListDirs(d.FullName));
        ...
        public async Task<IActionResult> FILESLIST(string d, string type)
        {
            try
            {
                d = await Task.Run(() => MakePhysicalPath(d));
                await  Task.Run(() =>CheckPath(d));
                string fullPath = await Task.Run(() =>FixPath(d));
                List<string> files = await Task.Run(() => GetFiles(fullPath, type));
                string result = "";
